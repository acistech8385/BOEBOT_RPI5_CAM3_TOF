 ...
 *   - tof_depth_YYYYMMDD_HHMMSS.csv      (raw depth values in mm)
 *   - tof_depth_YYYYMMDD_HHMMSS.png      (depth as JET colourmap, if cv2 available)
 *   - tof_amplitude_YYYYMMDD_HHMMSS.png  (amplitude greyscale, if cv2 available)
 *
...
public class ToFCaptureTest {
    ...
    private static final String TOF_CAPTURE_SCRIPT = """
            #!/usr/bin/env python3
            # BOEBOT ArduCam ToF Capture (auto-generated by BOEBOT Test App)
            import sys
            import os
            import time
            ...
            log("Opening ArduCam ToF camera (CAM1)...")
            ...
            ret, frame = cam.requestFrame(2000)
            if ret != 0 or frame is None:
                cam.stop()
                cam.close()
                fail("Failed to capture frame (error " + str(ret) + ")")
            ...
            if have_cv2:
                # Save depth as JET colour PNG
                d_n = cv2.normalize(depth, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
                d_png = os.path.join(save_dir, "tof_depth_" + timestamp + ".png")
                cv2.imwrite(d_png, cv2.applyColorMap(d_n, cv2.COLORMAP_JET))
                saved.append(d_png)
                print("SAVED:" + d_png, flush=True)

                # Save amplitude as greyscale PNG
                a_n = cv2.normalize(amp, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
                a_png = os.path.join(save_dir, "tof_amplitude_" + timestamp + ".png")
                cv2.imwrite(a_png, a_n)
                saved.append(a_png)
                print("SAVED:" + a_png, flush=True)
